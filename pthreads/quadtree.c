...
/* visit all leaf quadrants */
void visit_leaf_quadrant_range(struct quadtree *root, 
			       int start_lq, int end_lq,
			       QUADRANT_VISITOR_FN *func,
			       void *data) {
  int i;

  if (start_lq < 0) exit(1);
  
  if (is_leaf_quadrant(root)) {
    /* if a leaf, just need to make sure we are supposed to 
       visit leaf 0 */
    if (start_lq == 0) {
      (*func)(data, root);
    }
    ...
    /* for non-leaf quadrants, we need to figure out which child
       quadrants contain leaves we are to visit, and there will be 3
       cases: visit none, visit some, or visit all
    */
    // how many leaves came before the next one we'll visit?
    int before_this_leaf = 0;
    for (i=0; i<4; i++) {
      if (end_lq > before_this_leaf) {
	// only need to do anything more if we have not yet seen
	// a total of end_lq leaves in previous children
	int this_child_leaves = num_leaf_quadrants(root->children[i]);
	if (before_this_leaf + this_child_leaves > end_lq) {
	  // don't have to do everything in this child
	  visit_leaf_quadrant_range(root->children[i], 0,
				    end_lq - before_this_leaf, func, data);
	}
	else {
	  // will need to visit all in this one, so use the original
	  // version that does not need to check ranges
	  visit_all_leaf_quadrants(root->children[i], func, data);
	}
	before_this_leaf += this_child_leaves;
      }
    }
  }
}
